...
void playAirRaidSiren(int pin, int duration) {
    int stepDuration = 50;  // Duration of each frequency step in milliseconds
    int steps = duration / (2 * stepDuration);  // Total number of steps up and down

    // Ramp up the frequency
    for (int freq = SIREN_MIN; freq <= SIREN_MAX; freq += (SIREN_MAX - SIREN_MIN) / steps) {
        ...
        pthread_mutex_unlock(&lock);
        softToneWrite(pin, freq);  // Set the tone frequency
        delay(stepDuration);
    }

    // Ramp down the frequency
    for (int freq = SIREN_MAX; freq >= SIREN_MIN; freq -= (SIREN_MAX - SIREN_MIN) / steps) {
        pthread_mutex_lock(&lock);
        if (gas_level <= 230) {
            pthread_mutex_unlock(&lock);
            softToneWrite(pin, 0);  // Turn off the tone
...
void playLGJingle() {
    ...
    for (int i = 0; i < sizeof(melody)/sizeof(melody[0]); i++) {
        ...
        delay(durations[i]);
        softToneWrite(BUZZER_PIN, 0);  // Stop the tone for the duration between notes
        delay(EIGHT / 10);  // Short delay between notes
    }
}
...
void *buzzer_function(void *vargp) {
    ...
    while (1) {
        ...
        if (current_gas_level > 230) {  // Check if gas level is above threshold
            // Play air raid siren sound
            playAirRaidSiren(BUZZER_PIN, 2000);  // 2 seconds duration
        } else {
            // Play the LG jingle melody
            playLGJingle();
        }
